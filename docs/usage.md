...
If you prefer a lightweight genomic aligner, `--aligner bowtie2` provides a Bowtie2-based path that mirrors the existing HISAT2 branch. This route produces coordinate-sorted BAM files, feeds alignment QC metrics into MultiQC and, uniquely, generates two additional gene-level featureCounts tables per sample: one using all alignments and another restricted to reads meeting the `--featurecounts_min_mapping_quality` threshold (default Q≥3).
...
For similar reasons, quantification will not be performed if using `--aligner hisat2` due to the lack of an appropriate option to calculate accurate expression estimates from HISAT2 derived genomic alignments - this may change in future releases (see [#822](https://github.com/nf-core/rnaseq/issues/822)). HISAT2 has been made available for those who have a preference for the alignment, QC and other types of downstream analysis compatible with its output. Bowtie2 users receive per-sample gene-level featureCounts tables (for all and mapping-quality-filtered reads) alongside the standard biotype QC outputs.
...
- Tweak `--featurecounts_min_mapping_quality` (default `3`) to control the Bowtie2 unique-read gene-level featureCounts tables.
- Pass extra command-line switches to both Bowtie2 gene-level featureCounts runs with `--featurecounts_extra_args` (for example, supply `-M` to allow multi-mapping reads in the all-read table).
